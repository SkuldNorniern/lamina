...
mod tests {
    ...
    #[test]
    fn test_codegen_tensor_benchmark() -> Result<()> {
        // 1. Parse the benchmark file
        let source = fs::read_to_string("../../benchmarks/tensor benchmark/tensor_benchmark.lamina")?;
        ...
        let mut buffer = Vec::new();
        {
            ...
            // Call the correct top-level function
            x86_64::generate_x86_64_assembly(&module, &mut writer)?;
            writer.flush()?;
        }
        let assembly = String::from_utf8(buffer)?;

        // 3. Assertions ...
        assert!(assembly.contains("func_get_matrix_a_element:"), "Missing @get_matrix_a_element function");
        assert!(assembly.contains("func_get_matrix_b_element:"), "Missing @get_matrix_b_element function");
        assert!(assembly.contains("func_compute_matrix_cell:"), "Missing @compute_matrix_cell function");
        assert!(assembly.contains("func_matmul_2d:"), "Missing @matmul_2d function");
        assert!(assembly.contains("main:"), "Missing main function"); // Exported main doesn't get func_ prefix
        assert!(assembly.contains("call func_compute_matrix_cell"), "Missing call to compute_matrix_cell in matmul_2d");
        assert!(assembly.contains("call func_get_matrix_a_element"), "Missing call to get_matrix_a_element in compute_matrix_cell");
        assert!(assembly.contains("call func_get_matrix_b_element"), "Missing call to get_matrix_b_element in compute_matrix_cell");
        assert!(assembly.contains("call func_matmul_2d"), "Missing call to matmul_2d in main");
        assert!(assembly.contains("call printf@PLT"), "Missing calls to printf");
        assert!(assembly.contains("# IR: %result_size = mul.i64 %n_rows, %n_cols"), "Missing result_size calculation IR comment");
        assert!(assembly.contains("imulq"), "Expected imulq instruction(s)");
        assert!(assembly.contains("# IR: %progress_step = div.i64 %result_size, 20"), "Missing progress_step calculation IR comment");
        assert!(assembly.contains("idivq"), "Expected idivq instruction(s)");
        assert!(assembly.contains("# IR: %i_done = eq.i64 %i, %n_rows"), "Missing row loop condition IR comment");
        assert!(assembly.contains("cmpq"), "Expected cmpq instruction(s)");
        assert!(assembly.contains("sete"), "Expected sete instruction(s)");
        assert!(assembly.contains("jne"), "Expected conditional jump(s)");
        assert!(assembly.contains(".section .rodata"), "Missing .rodata section");
        assert!(assembly.contains("%lld\n"), "Missing format string %lld\n");
        assert!(assembly.contains("pushq %rbp"), "Missing function prologue (push rbp)");
        assert!(assembly.contains("movq %rsp, %rbp"), "Missing function prologue (mov rbp)");
        assert!(assembly.contains("subq $"), "Missing stack allocation (subq rsp)");
        assert!(assembly.contains("leave"), "Missing function epilogue (leave)");
        assert!(assembly.contains("ret"), "Missing function epilogue (ret)");
