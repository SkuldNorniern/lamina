...
mod tests_licm {
    ...
    fn test_licm_basic() {
        ...
        let mut func = FunctionBuilder::new("test_licm")
            ...
            .block("entry")
            .instr(Instruction::IntBinary { op: IntBinOp::Add, ty: MirType::Scalar(ScalarType::I64), dst: VirtualReg::gpr(0).into(), lhs: Operand::Immediate(Immediate::I64(0)), rhs: Operand::Immediate(Immediate::I64(0)) }) // init v0
            .instr(Instruction::Jmp { target: "loop".to_string() })
            .block("loop")
        ...
        assert!(!has_invariant, "Invariant instruction should be moved out of loop");
...
impl LoopInvariantCodeMotion {
            ...
            for instr in &block.instructions {
                ...
                } = instr
                {
                    ...
                    if self.is_back_edge(&dominators, &block.label, true_target)
                        && let Some(loop_info) = self.analyze_loop(func, true_target, &block.label, &dominators)
                    {
                        loops.push(loop_info);
                    }
                    if self.is_back_edge(&dominators, &block.label, false_target)
                        && let Some(loop_info) = self.analyze_loop(func, false_target, &block.label, &dominators)
                    {
                        loops.push(loop_info);
                    }
                }
                if let Instruction::Jmp { target } = instr
                    ...
                    && let Some(loop_info) = self.analyze_loop(func, target, &block.label, &dominators)
                {
                    loops.push(loop_info);
                }
            }
    ...
    fn is_back_edge(&self, dominators: &HashMap<String, HashSet<String>>, from: &str, to: &str) -> bool {
    ...
    fn calculate_dominators(&self, func: &Function) -> HashMap<String, HashSet<String>> {
        ...
        while changed {
            ...
            for block in &func.blocks {
                ...
                // Find predecessors
                let preds: Vec<&Block> = func.blocks.iter().filter(|pred| {
                    self.has_edge_to(func, &pred.label, &block.label)
                }).collect();
...
impl LoopUnrolling {
    ...
    fn find_unrollable_loops(&self, func: &Function) -> Vec<UnrollableLoop> {
        ...
        // Map label -> index
        let label_to_idx: std::collections::HashMap<_, _> = func.blocks.iter()
            .enumerate()
        ...
        for (idx, block) in func.blocks.iter().enumerate() {
            if let Some(terminator) = block.instructions.last() {
                let targets = match terminator {
                    ...
                    Instruction::Br { true_target, false_target, .. } => vec![true_target, false_target],
                    _ => vec![],
                };
                ...
                for target in targets {
                    ...
                    if let Some(&target_idx) = label_to_idx.get(target.as_str()) {
                         if target_idx <= idx {
                             ...
                             if let Some((bound, exit_target)) = self.analyze_loop(func, target, &block.label) {
                                 ...
                                 if !body_blocks.is_empty() && body_blocks.len() <= 10 { // Max 10 blocks in body
    ...
    fn analyze_loop(&self, func: &Function, header_label: &str, latch_label: &str) -> Option<(i64, String)> {
        ...
        let (cond_reg, true_target, false_target) = match latch_block.instructions.last()? {
            Instruction::Br { cond, true_target, false_target, .. } => (cond, true_target, false_target),
            Instruction::Jmp { target } if target == header_label => {
                ...
                match header_block.instructions.last()? {
                    Instruction::Br { cond, true_target, false_target, .. } => {
                        return self.analyze_bound_from_cond(func, header_block, cond, true_target, false_target, header_label);
                    }
                    _ => return None,
                }
            }
        ...
        self.analyze_bound_from_cond(func, latch_block, cond_reg, true_target, false_target, header_label)
    }
    ...
    fn analyze_bound_from_cond(
        ...
        let cond_def = block.instructions.iter().find(|i| i.def_reg() == Some(cond_reg))?;
        
        if let Instruction::IntCmp { op, lhs, rhs, .. } = cond_def {
             // Expect induction variable vs Constant
             let (reg, limit) = match (lhs, rhs) {
                 (Operand::Register(r), Operand::Immediate(crate::mir::Immediate::I64(c))) => (r, *c),
             ...
             let inc_def = block.instructions.iter().find(|i| i.def_reg() == Some(reg))?;
             if let Instruction::IntBinary { op: IntBinOp::Add, lhs: Operand::Register(src), rhs: Operand::Immediate(crate::mir::Immediate::I64(step)), .. } = inc_def {
                 if src == reg && *step == 1 {
                     // i++ loop.
                     // Standard "while (i < N)"
    ...
    fn collect_loop_body(&self, func: &Function, header: &str, latch: &str) -> Vec<String> {
        ...
        let order: std::collections::HashMap<_, _> = func.blocks.iter().enumerate().map(|(i, b)| (b.label.clone(), i)).collect();
    ...
    fn unroll_loop(&self, func: &mut Function, loop_info: &UnrollableLoop) -> Result<bool, String> {
                         ...
                         Instruction::Br { true_target, false_target, .. } => {
                             ...
                             if false_target == &loop_info.header {
                                 if i == loop_info.bound - 1 {
                                     ...
                                     *false_target = format!("{}_unroll_{}", loop_info.header, i + 1);
                                 }
         ...
         for block in &mut func.blocks {
             ...
             for instr in &mut block.instructions {
                 match instr {
                     ...
                     Instruction::Br { true_target, false_target, .. } => {
                         if true_target == &loop_info.header { *true_target = entry_label.clone(); }
                         if false_target == &loop_info.header { *false_target = entry_label.clone(); }
                     }
                     _ => {}
                 }
             }
         ...
         for block in &mut func.blocks {
             if block.label.starts_with(&format!("{}_unroll_", loop_info.header)) {
...
mod tests {
    ...
    fn test_loop_unrolling_simple() {
        ...
        let loop_0 = func.get_block("loop_unroll_0").expect("loop_unroll_0 exists");
        ...
        let loop_3 = func.get_block("loop_unroll_3").expect("loop_unroll_3 exists");
