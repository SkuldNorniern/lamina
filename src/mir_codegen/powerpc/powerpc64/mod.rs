...
fn emit_instruction_ppc64<W: Write>(
    ...
    match inst {
        ...
        MirInst::FloatBinary {
            ...
            match op {
                FloatBinOp::FAdd => writeln!(writer, "    fadd{} 1, 1, 2", suffix)?,
                FloatBinOp::FSub => writeln!(writer, "    fsub{} 1, 1, 2", suffix)?,
                FloatBinOp::FMul => writeln!(writer, "    fmul{} 1, 1, 2", suffix)?,
                FloatBinOp::FDiv => writeln!(writer, "    fdiv{} 1, 1, 2", suffix)?,
            }
            if let Register::Virtual(vreg) = dst {
        ...
        MirInst::FloatUnary { op, dst, src, ty } => {
            ...
            match op {
                FloatUnOp::FNeg => writeln!(writer, "    fneg{} 1, 1", suffix)?,
                FloatUnOp::FSqrt => writeln!(writer, "    fsqrt{} 1, 1", suffix)?,
            }
            if let Register::Virtual(vreg) = dst {
                emit_store_fp_result("1", vreg, writer, reg_alloc, stack_slots, is_f32)?;
            }
