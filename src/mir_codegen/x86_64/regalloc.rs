...
///
/// Strategy mirrors the AArch64 helper:
...
// Conservative mapping pool (mostly callee-saved under SysV): r12–r15, rbx
// Avoid rsp/rbp, arg/ret regs, and commonly clobbered caller-saved regs.
const MAP_GPRS: &[&str] = &["r12", "r13", "r14", "r15", "rbx"];

// Dedicated scratch pool for ephemeral use during emission.
// Keep disjoint from MAP_GPRS. r10/r11 are caller-saved temporaries on SysV.
const SCRATCH_GPRS: &[&str] = &["r10", "r11"];
