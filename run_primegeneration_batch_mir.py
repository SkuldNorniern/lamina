...
# --- Configuration ---
BENCHMARK_DIR = Path("benchmarks/primegeneration") # Use Path for directory handling
LAMINA_EXECUTABLE = "./target/release/lamina"
LAMINA_SOURCE = "benchmarks/primegeneration/primegeneration.lamina" # Keep lamina source path relative to root
LAMINA_OUTPUT_BINARY = "primegeneration_lamina" # Consistent naming

# Determine target OS for assembly generation
import platform
LAMINA_FLAG = ["--emit-mir-asm","--opt-level", "3"]
BATCH_RUNS = 250  # Number of times to run each benchmark (fewer since prime generation is slower)
TIME_THRESHOLD = 2.0  # Seconds - prime generation can be slower
...
def create_csharp_project_file(benchmark_dir):
    ...
    csproj_path = benchmark_dir / "primegeneration.csproj"
  ...
  <PropertyGroup>
    ...
    <AssemblyName>PrimeGeneration</AssemblyName>
...
def compile_and_run(target, results, cwd):
    ...
    if compile_cmd_template:
        ...
        if label == "C#" and isinstance(compile_cmd_template, list) and len(compile_cmd_template) > 1:
            ...
            create_cmd = ['dotnet', 'new', 'console', '-n', 'PrimeGeneration', '--force', '--no-restore']
            ...
            build_cmd = ['dotnet', 'build', '-c', 'Release', 'primegeneration.csproj']
    ...
    if label == "Java":
        # run_cmd_template is already correctly defined as ['java', 'PrimeGeneration']
    ...
    # Handle C# special case
        ...
        if output_path_rel:
            exec_path_rel = f"./{output_path_rel}" # e.g., ./bin/Release/net9.0/PrimeGeneration
    ...
    # Handle other compiled languages
        ...
        exec_path_rel = f"./{output_path_rel}" # e.g., ./primegeneration_c or ./primegeneration
...
def main():
    ...
    print("===== Multi-Language Prime Generation Benchmark =====")
    ...
    artifacts_to_clean = [
        "primegeneration_lamina", # Lamina output at root
        BENCHMARK_DIR / "primegeneration_c",
        BENCHMARK_DIR / "primegeneration_cpp",
        BENCHMARK_DIR / "primegeneration_rs",
        BENCHMARK_DIR / "primegeneration_go",
        BENCHMARK_DIR / "primegeneration_zig",
        BENCHMARK_DIR / "PrimeGeneration.class",
        BENCHMARK_DIR / "bin", # C# output directory
        BENCHMARK_DIR / "primegeneration.csproj", # Temporary C# project file
    ...
    if not test_single_lang or test_single_lang.lower() == "lamina":
        ...
        results['Lamina'] = {'time': 1.0, 'exit_code': 0, 'memory': 100.0, 'single_run': True, 'times': [1.0], 'outliers': 0, 'version': 'Unknown'}
    ...
    targets = [
        {'lang': 'C',     'source': 'primegeneration.c', 'output': 'primegeneration_c',
         'compile': ['gcc', '-O3', '-o', '{out}', '{src}'], 'run': ['./{out}'],
         'version_cmd': ['gcc', '--version']},
        {'lang': 'C++',   'source': 'primegeneration.cpp', 'output': 'primegeneration_cpp',
         'compile': ['g++', '-O3', '-o', '{out}', '{src}'], 'run': ['./{out}'],
         'version_cmd': ['g++', '--version']},
         {'lang': 'Rust',  'source': 'primegeneration.rs', 'output': 'primegeneration_rs',
          'compile': ['rustc', '-C', 'opt-level=3', '--out-dir', '.', '-o', '{out}', '{src}'], 'run': ['./{out}'],
          'version_cmd': ['rustc', '--version']},
        {'lang': 'Go',    'source': 'primegeneration.go', 'output': 'primegeneration_go',
         'compile': ['go', 'build', '-o', '{out}', '{src}'], 'run': ['./{out}'],
         'version_cmd': ['go', 'version']},
        {'lang': 'Zig',   'source': 'primegeneration.zig', 'output': 'primegeneration_zig',
         'compile': ['zig', 'build-exe', '{src}', '-O', 'ReleaseFast', '--name', '{out}'], 'run': ['./{out}'],
         'version_cmd': ['zig', 'version']},
        {'lang': 'Nim',   'source': 'primegeneration.nim', 'output': 'primegeneration',
         'compile': ['nim', 'c', '-d:release', '--out:{out}', '{src}'], 'run': ['./{out}'],
         'version_cmd': ['nim', '--version']},
        {'lang': 'Java',  'source': 'PrimeGeneration.java', 'output': 'PrimeGeneration.class', # Compile produces .class
         'compile': ['javac', '{src}'],
         # Run needs class name (no extension), executed from BENCHMARK_DIR
         'run': ['java', 'PrimeGeneration'],
         'version_cmd': ['java', '--version']},
        # Updated C# configuration to build and then run the executable directly
        {'lang': 'C#', 'source': 'primegeneration.cs', 'output': 'bin/Release/net9.0/PrimeGeneration',
        ...
        {'lang': 'Python', 'source': 'primegeneration.py', 'run': ['python3', '{src}'],
         'version_cmd': ['python3', '--version']},
        {'lang': 'JavaScript', 'source': 'primegeneration.js', 'run': ['node', '{src}'],
         'version_cmd': ['node', '--version']},
        {'lang': 'Ruby', 'source': 'primegeneration.rb', 'run': ['ruby', '{src}'],
         'version_cmd': ['ruby', '--version']},
        {'lang': 'PHP', 'source': 'primegeneration.php', 'run': ['php', '{src}'],
    ...
    print(f"{V}{f'Prime Generation Benchmark Results (Higher ratio is Better for Lamina) [{BATCH_RUNS} runs]':^{total_width}}{V}")
